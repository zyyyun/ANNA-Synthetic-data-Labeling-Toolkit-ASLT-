...
**Final Build:** ASLT-Setup-v1.0.3.exe (98.19 MB, SHA256 `D139CD900A36F2CB098DB66002CC82D4E61CA84FC0F90FEC547FA577C028B496` — 5차 final, 2026-05-07 16:39:11)
...
5. **사용자 흐름 개선 (out-of-scope but in-binary)** — 정보 팝업 Entry/Exit Shift 변형 표기 (commit `8a22445`), 로그 보존 30일 → 180일 (commit `df890bd`), 시작 가이드(OnboardingForm) Entry/Exit Shift 변형 표기 (commit `f719195`, 4차 빌드 cycle)
6. **FUNC-12 implementation gap closure** — re-tag 직후 사용자가 close/switch 자동 저장 path unguarded 발견. `TryGuardOneBoxSave()` 헬퍼 추출 + 영상 전환 / 폼 종료 자동 저장 path 보호 (commit `29be68e`, 5차 빌드 cycle). FUNC-12 acceptance generic "JSON 저장 시" 가 모든 save 진입점에서 균일 보호.
...
- 3차/4차/5차 빌드 cycle (UAT 도중 popup/retention 변경, closeout 직후 OnboardingForm 변경, re-tag 직후 FUNC-12 implementation gap closure) — 향후 milestone 에서 UAT 단계 진입 전 source freeze 정책 명시 필요. closeout 후 re-open 은 tag 미배포 상태에서만 안전 (배포 후엔 v1.0.4 patch 로 처리해야 함). 5차 cycle 의 FUNC-12 보강 사례는 acceptance criteria 가 generic 했지만 implementation 이 한 path 에만 적용된 gap 패턴 — 다음 milestone 에서 acceptance ↔ test path 매핑을 명시 검증할 RELI 케이스 추가 권장.
