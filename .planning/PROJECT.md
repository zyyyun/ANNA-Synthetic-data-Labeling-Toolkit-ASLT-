...
**Active milestone:** v1.0.3 — JSON 저장 결함 수정 (KTC 2차 결함보고서)
**Status:** v1.0 사이클 종료 후 KTC 2차 결함보고서 후속 patch 사이클 진행 중
**Coverage:** v1 요구사항 43개 중 39 완료 (90.7%). DOC-01/02/03 + USAB-05 는 Known Gaps (별도 milestone)
...
## Current Milestone: v1.0.3 — JSON 저장 결함 수정 (KTC 2차)

**Goal:** KTC 2차 결함보고서에서 식별된 JSON 저장 관련 결함 2건을 수정하여 인증 신청 전 결함 0건 상태를 유지한다.

**Target defects (KTC 2차):**
- **DF-2-05** — JSON 저장 시 '전체선택(1번)' 모드가 활성화되어야 하나 좌측 버튼은 편집모드(0번, 파란색)로 표시되어 시각 표시 불일치 발생 (기능 적합성, 결함 정도: 중간)
- **DF-2-06** — BBOX 1개만 생성된 상태에서 JSON 저장 시 Entry/Exit 가 동일 프레임으로 설정되어 Waypoint 생성됨 (기능 적합성, 결함 정도: 중간)

**Out of scope (이번 milestone):**
- DOC-01/02/03, USAB-05, AVI cold-start, HumanUAT 잔여 — 별도 milestone

> 다음 milestone (DOC + 인증 신청 직전 정리) 은 v1.0.3 종료 후 `/gsd:new-milestone` 으로 정의.
...
KTC 2차 결함보고서 — JSON 저장 결함 2건. 기능적합성 카테고리에 추가. 상세는 [REQUIREMENTS.md](REQUIREMENTS.md):

- **FUNC-11**: JSON 저장 시 '전체선택(1번)' 모드 활성화 시 좌측 버튼의 시각 표시도 '전체선택' 으로 동기화 (편집모드 잔존 표시 금지) — DF-2-05
- **FUNC-12**: BBOX 1개만 생성된 상태에서 JSON 저장 시 Entry/Exit 가 동일 프레임으로 설정되어 Waypoint 생성되는 동작 차단 — DF-2-06
...
---
*Last updated: 2026-05-06 — v1.0.3 milestone (JSON 저장 결함 수정) 시작*
