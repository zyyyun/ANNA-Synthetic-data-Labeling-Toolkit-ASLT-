...
**Shipped:** v1.0.3 (2026-05-07, 4차 final) — ASLT-Setup-v1.0.3.exe (98.19 MB, SHA256 `2072B5B5...`)
...
- ✓ BBOX 1개 상태 JSON 저장 가드 — Entry==Exit Waypoint 자동 생성 차단 — v1.0.3 (Phase 7-02, FUNC-12 / DF-2-06)
...
| 3차/4차 빌드 cycle (UAT 도중 popup + retention, closeout 직후 OnboardingForm) | 사용자 흐름 개선 사항을 별도 milestone 으로 미루지 않고 v1.0.3 동일 binary 통합. tag 미배포 상태에서 closeout 후 re-open 안전 | ⚠ Revisit — 향후 milestone 에서 UAT 진입 전 source freeze 정책 명시 필요. tag push 이후엔 re-open 금지 → v1.0.x patch 로 처리 |
...
---
*Last updated: 2026-05-07 after v1.0.3 4차 빌드 cycle (OnboardingForm Shift 변형 일관성 patch). JSON 저장 결함 수정 (KTC 2차) shipped, ASLT-Setup-v1.0.3.exe production-ready (SHA256 `2072B5B5...`).*
